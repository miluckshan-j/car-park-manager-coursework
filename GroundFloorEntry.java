        ...
        while (true) {
            try {
                ...
                if (randomNumber < 0.142) {
                    // Car
                    vehicle = new Car("WP-" + randomNumberGenerator(0, 50), "Nissan", "Leaf", entry, 4, Color.WHITE);
                } else if (randomNumber < 0.285) {
                    // Van
                    vehicle = new Van("WP-" + randomNumberGenerator(0, 50), "Toyota", "Hiace", entry, 8);
                } else if (randomNumber < 0.428) {
                    // Motor Bike
                    vehicle = new MotorBike("WP-" + randomNumberGenerator(0, 50), "TVS", "Platina", entry, "1500CC");
                } else if (randomNumber < 0.571) {
                    // Lorry
                    vehicle = new Lorry("WP-" + randomNumberGenerator(0, 50), "Mitsubishi", "Canter", entry, 1000.0, 5.5);
                } else if (randomNumber < 0.714) {
                    // Mini Lorry
                    vehicle = new MiniLorry("WP-" + randomNumberGenerator(0, 50), "Dimo", "Batta", entry, 500.0, 4.0);
                } else if (randomNumber < 0.857) {
                    // Bus
                    vehicle = new Bus("WP-" + randomNumberGenerator(0, 50), "Tata", "Leyland", entry, 55, 4.0);
                } else {
                    // Mini Bus
                    vehicle = new MiniBus("WP-" + randomNumberGenerator(0, 50), "Toyota", "Coaster", entry, 30, 3.0);
                }
